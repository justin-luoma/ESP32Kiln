...
String Preferences_parser(const String &var) {
  ...
  else if (var == "PID_Auto_Setpoint") return String(Prefs[PRF_PID_AUTO_SETPOINT].value.vfloat);
  ...
  else if (var == "PID_Measure_Interval") return String(Prefs[PRF_PID_MEASURE_INTERVAL].value.uint16);
...
String Chart_parser(const String &var) {
  ...
  if (var == "CHART_DATA" && Program_run_size > 0) {
    ...
    str[strlen(str) - 1] = '\0';  // Dont know why - probably error, but ctime returns string with new line char and tab - so we cut it off
    tmp += "{x:'" + String(str) + "'";
